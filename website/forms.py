...
class LoginForm(FlaskForm):
    ...
    def validate_username(self, username_to_check):
        ...
        if user:
            raise ValidationError(
                'Username aldready exists!, Please try a different username')
    ...
    def validate_email_address(self, email_address_to_check):
        ...
        if email_address:
            raise ValidationError(
                'Email Address aldready exists! Please try a different email address')
    ...
    def validate_phone_number(self, phone_number_to_check):
        ...
        if phone_number:
            raise ValidationError(
                'Phone number aldready exists! Please try a different email address')

    username = StringField(label='User Name', validators=[
                           Length(min=2, max=30), DataRequired()])
    email_address = StringField(label='Email Address', validators=[
                                Email(), DataRequired()])
    phone_number = StringField(label='Phone Number', validators=[Length(
        min=10, max=12), DataRequired(), Regexp(regex='^[+-]?[0-9]+$')])
    password1 = PasswordField(label='Password', validators=[
                              Length(min=6), DataRequired()])
    password2 = PasswordField(label='Confirm Password', validators=[
                              EqualTo('password1'), DataRequired()])
    address = StringField(label='Address (all deliveries will be done to this address)', validators=[
                          Length(min=20), DataRequired()])
